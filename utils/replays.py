...
class ReplayBuffer:
    ...
    def add_batch(self, obses, actions, rewards, next_obses, not_dones):
        for obs, action, reward, next_obs, not_done in zip(
    ...
    def sample_traj(self, batch_size, episode_length, horizon):
        ...
        traj_slices = np.random.choice(
            episode_length - horizon, size=len(eoo), replace=False
        )
        ...
        indexes = np.array(
            [
                (
                    indexes[eoo[i] + 1 : eoo[i + 1]][
                        traj_slices[i] : traj_slices[i] + horizon
                    ]
                    if i > 0
                    else indexes[eoo[i] : eoo[i + 1]][
                        traj_slices[i] : traj_slices[i] + horizon
                    ]
                )
    ...
    def random_split(self, val_size, batch_size=None):
        ...
        if batch_size is not None:
            ...
            training_batch = (
                ...
                torch.FloatTensor(self.next_states[batch_idxs[val_size:]]).to(
                    self.device
                ),
                torch.FloatTensor(self.rewards[batch_idxs[val_size:]]).to(self.device),
                torch.FloatTensor(self.not_dones[batch_idxs[val_size:]]).to(
                    self.device
                ),
            )
...
class OfflineReplay:
    def __init__(self, dataset, device, custom_filepath=None):
        self.device = device
        # If a custom file path is provided, load the dataset from the JSON file.
        if custom_filepath:
            with open(custom_filepath, "r") as f:
                dataset = json.load(f)
        ...
        self.next_states = np.array(dataset["next_observations"])
        self.rewards = np.array(dataset["rewards"]).reshape(-1, 1)

        # Use 'terminals' if available; otherwise check for 'dones'; default to zeros.
        if "terminals" in dataset:
            terminals = np.array(dataset["terminals"])
        elif "dones" in dataset:
            terminals = np.array(dataset["dones"])
        else:
            terminals = np.zeros(len(self.rewards))
        self.not_dones = (1 - terminals).reshape(-1, 1)

        self.size = self.rewards.shape[0]

    def random_split(self, val_size, batch_size):
        # Select indices where the episode is not done.
        valid_indices = np.where(self.not_dones.reshape(-1) == 1)[0]
        batch_idxs = np.random.permutation(valid_indices)[:batch_size]
    ...
    def sample(self, batch_size, rl=False):
        if not rl:
            valid_indices = np.where(self.not_dones.reshape(-1) == 1)[0]
            ind = np.random.choice(valid_indices, size=batch_size)
        else:
            ind = np.random.randint(0, self.size, size=batch_size)
