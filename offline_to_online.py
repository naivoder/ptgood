...
base_model_buffer_size = int(
    model_retain_epochs * args.rollout_batch_size * epoch_length / args.model_train_freq
)
...
if args.model_file:
    ...
    while early_stop_counter < early_stop:
        ...
        while e_idx <= dynamics_ens.replay.size:
            ...
            if np.all(
                [b_idx < dynamics_ens.replay.size, e_idx > dynamics_ens.replay.size]
            ):
            ...
            for j in range(25000):
                ...
                agent.update(
                    preprocess_sac_batch(
                        offline_replay, model_replay_buffer, rl_batch_size, real_ratio
                    ),
                    j,
                    args.loss_penalty,
                    None,
            ...
            if online_steps % args.model_train_freq == 0:
                ...
                while early_stop < early_stop_ckpt:
                    ...
                    while e_idx <= dynamics_ens.replay.size:
                        ...
                        state_err = (
                            (means - train_targets)[:, :-1].pow(2).mean().cpu().item()
                        )
                        reward_err = (
                            (means - train_targets)[:, -1].pow(2).mean().cpu().item()
                        )
            ...
            if online_steps % 1000 == 0:
                ...
                for _ in range(n_eval_episodes):
                    ...
                    while not done:
                        ...
                        means = torch.cat(means, dim=0)
                        disagreement = (
                            (torch.norm(means - means.mean(0), dim=-1)).mean(0).item()
                        )
                        wandb.log({"disagreement": disagreement})
