...
def convert_minari_to_dict(dataset):
    """
    Converts a MinariDataset into a dictionary with keys:
    "observations", "actions", "next_observations", "rewards", "terminals".
    Uses dataset.iterate_episodes() to loop over episodes and combines terminations
    and truncations into a done flag.
    """
    all_obs = []
    all_actions = []
    all_rewards = []
    ...
    for episode_data in dataset.iterate_episodes():
        ...
        # Combine terminations and truncations to form done flags
        dones = np.logical_or(terminations, truncations).astype(np.int32)

        # Create next_obs by shifting; duplicate last observation if needed.
        if len(obs) > 1:
...
def main():
    ...
    model_replay_buffer = ReplayBuffer(
        max_model_buffer_size, state_dim, action_dim, device
    )
    ...
    if (
        ...
        or "pen" in args.env
        or "hammer" in args.env
        or "door" in args.env
        or "relocate" in args.env
        or "quadruped" in args.env
        or "kitchen" in args.env
    ):
        bs = 1024
        dynamics_hidden = (
            200 if ("cmu" in args.env.lower() or "escape" in args.env.lower()) else 400
        )
    else:
        bs = 256
        dynamics_hidden = 200
    ...
    dynamics_ens = DynamicsEnsemble(
        ...
        state_dim,
        action_dim,
        [dynamics_hidden for _ in range(4)],
        "elu",
        False,
        "normal",
        5000,
    ...
    minari_dict = convert_minari_to_dict(minari_dataset)
    # FIX: Pass the converted dictionary instead of the raw dataset.
    offline_replay = OfflineReplay(minari_dict, device, args.custom_filepath)
    ...
    if args.ceb_planner:
        planner = CEBVecTree(
            lambda_q=0.0, lambda_r=1.0, noise_std=args.ceb_planner_noise
        )
    ...
    if args.model_file:
        ...
        while early_stop_counter < early_stop:
            ...
            while e_idx <= dynamics_ens.replay.size:
                ...
                train_inputs, train_targets = dynamics_ens.preprocess_training_batch(
                    train_batch
                )
                train_inputs = dynamics_ens.scaler.transform(train_inputs)

                with torch.no_grad():
                    means, _ = dynamics_ens.forward_models[
                ...
                e_idx += batch_size
                if (
                    b_idx < dynamics_ens.replay.size
                    and e_idx > dynamics_ens.replay.size
                ):
    ...
    if not args.rl_file:
        print(
            f"No RL file given. Starting policy pre-training from offline dataset...\n"
        )
            ...
            while offline_pretraining_step <= args.offline_steps:
                ...
                if args.eval_model:
                    ...
                    for _ in range(n_eval_episodes):
                        ...
                        episode_reward = 0
                        while not done:
                            action, dist = agent.act(
                                eval_obs, sample=False, return_dist=True
                            )
                            eval_next_obs, reward, terminated, truncated, info = (
                                env.step(action)
                            )
                            ...
                            next_state_pred = (
                                model_pred[:, :-1].cpu().numpy() + eval_obs
                            )
                            model_errors_s.append(
                                ((next_state_pred - eval_next_obs) ** 2).mean()
                            )
                            model_errors_r.append(
                                ((model_pred[:, -1].cpu().numpy() - reward) ** 2).mean()
                            )
                    ...
                    for _ in range(n_eval_episodes):
                        ...
                        while not done:
                            ...
                            if any(
                                x in args.env
                                for x in ["pen", "hammer", "relocate", "door"]
                            ):
                ...
                wandb.log(
                    {
                        "offline_step": offline_pretraining_step,
                        "offline_eval_returns": np.mean(eval_rewards),
                    }
                )
    ...
    for _ in range(n_eval_episodes):
        ...
        while not done:
            ...
            eval_next_obs, reward, terminated, truncated, info = eval_env.step(action)
            done = terminated or truncated
        ...
        while online_steps <= args.online_steps:
            ...
            while not done:
                ...
                if online_steps % args.model_train_freq == 0:
                    train_batch, _ = offline_replay.random_split(0, offline_replay.size)
                    online_batch, _ = online_replay_buffer.random_split(
                        0, online_replay_buffer.size
                    )
                    train_batch = [
                        torch.cat((env_item, model_item), dim=0)
                        for env_item, model_item in zip(train_batch, online_batch)
                    ]
                    train_inputs, _ = dynamics_ens.preprocess_training_batch(
                        train_batch
                    )
                    dynamics_ens.scaler.fit(train_inputs)
                    ...
                    while early_stop < early_stop_ckpt:
                        loss_hist = dynamics_ens.train_single_step(
                            dynamics_ens.replay, 0.2, bs, online_replay_buffer
                        )
                        ...
                        while e_idx <= dynamics_ens.replay.size:
                            ...
                            train_inputs, train_targets = (
                                dynamics_ens.preprocess_training_batch(train_batch)
                            )
                            train_inputs = dynamics_ens.scaler.transform(train_inputs)
                        ...
                        while e_idx <= online_replay_buffer.size:
                            ...
                            e_idx += batch_size
                            if (
                                b_idx < online_replay_buffer.size
                                and e_idx > online_replay_buffer.size
                            ):
                ...
                if online_steps % args.ceb_update_freq == 0:
                    ...
                    online_batch, _ = model_replay_buffer.random_split(
                        0, model_replay_buffer.size
                    )
                    s, a, *_ = online_batch
                    sa = torch.cat([s, a], dim=-1)
                    ...
                    for _ in range(50_000):
                        ceb_step_hist = ceb.train_step(
                            512, model_replay_buffer, scaler=ceb.scaler
                        )
                ...
                if online_steps % 1000 == 0:
                    ...
                    for _ in range(n_eval_episodes):
                        ...
                        while not done:
                            ...
                            means = torch.cat(means, dim=0)
                            disagreement = (
                                (torch.norm(means - means.mean(0), dim=-1))
                                .mean(0)
                                .item()
                            )
                            wandb.log({"disagreement": disagreement})
                            ...
                            if any(
                                x in args.env
                                for x in ["pen", "door", "hammer", "relocate"]
                            ):
                                if info.get("goal_achieved", False):
                                    episode_reward = 1
                                    done = True
                        ...
                        eval_rewards.append(episode_reward)
                    wandb.log(
                        {"step": online_steps, "eval_returns": np.mean(eval_rewards)}
                    )
